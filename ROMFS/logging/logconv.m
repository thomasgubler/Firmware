...
function DrawRawData() 
    ...
    title(h.axes(3),'Accelerometers [m/s\B2]');
    ...
    title(h.axes(8),'Actuator PWM (raw-)outputs [\B5s]');
...
function DrawCurrentAircraftState()
    ...
    %**********************************************************************
    acstate{1,:}=[sprintf('%s \t\t','GPS Pos:'),'[lat=',num2str(double(sysvector.gps_raw_position(i,1))*fconv_gpslatlong),'\B0, ',...
                        'lon=',num2str(double(sysvector.gps_raw_position(i,2))*fconv_gpslatlong),'\B0, ',...
    ...
    acstate{3,:}=[sprintf('%s \t\t','Accels[m/s\B2]'),'[x=',num2str(sysvector.accel(i,1)),...
    ...
    acstate{7,:}=[acstate{7,:},']'];
    acstate{8,:}=sprintf('%s \t[','Actuator Outputs [PWM/\B5s]:');
    for j=1:8
